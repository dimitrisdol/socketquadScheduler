// Package hardcoded contains an implementation of socketquad.InterferenceModel,
...
// HardcodedSlowDowns is an implementation of socketquad.InterferenceModel, where the
// slowdowns among all applications are known and hardcoded.
type HardcodedSlowDowns struct {
	socketquadLabelKey string
}

// New returns a new HardcodedSlowDowns with the given label key (the one that
// is used by SocketQuadPlugin to track its applications).
func New(socketquadLabelKey string) *HardcodedSlowDowns {
	return &HardcodedSlowDowns{
		socketquadLabelKey: socketquadLabelKey,
	}
}

// Attack implements socketquad.InterferenceModel; see the documentation there for
// more information.
func (m *HardcodedSlowDowns) Attack(attacker, occupant *corev1.Pod) (float64, error) {
	occPodCategory, _ := parseAppCategory(occupant.Labels[m.socketquadLabelKey])
	//  occupant   ^^^   Pod's label's value must have been
	// validated back when it was first scheduled on the Node
	newPodCategory, err := parseAppCategory(attacker.Labels[m.socketquadLabelKey])
	if err != nil {
		return -1, err
	}
...
// ToInt64Multiplier implements socketquad.InterferenceModel; see the documentation
...
// the 4 appCategories that are to be examined by the SocketQuadPlugin are as
// follows:
...
// slowDownMatrix is a type alias for internal use in SocketQuadPlugin.
