// Package socketquad contains an out-of-tree plugin based on the Kubernetes
...
import (
	...
	"github.com/dimitrisdol/socketquadScheduler/socketquad/hardcoded"
...
var (
	_ framework.Plugin          = &SocketQuadPlugin{}
	_ framework.FilterPlugin    = &SocketQuadPlugin{}
	_ framework.ScorePlugin     = &SocketQuadPlugin{}
	_ framework.ScoreExtensions = &SocketQuadPlugin{}
)

// New instantiates a SocketQuadPlugin.
func New(configuration runtime.Object, f framework.Handle) (framework.Plugin, error) {
	return &SocketQuadPlugin{
		handle: f,
		model:  hardcoded.New(socketquadLabelKey),
	}, nil
}
...
// findCurrentOccupants returns all Pods that are being tracked by SocketQuadPlugin
...
func (_ *SocketQuadPlugin) findCurrentOccupants(nodeInfo *framework.NodeInfo) []*corev1.Pod {
	ret := make([]*corev1.Pod, 0, 4)
	for _, podInfo := range nodeInfo.Pods {
		for key := range podInfo.Pod.Labels {
			if socketquadLabelKey == key {
...
func (ap *SocketQuadPlugin) Filter(
	ctx context.Context,
	...
	if _, exists := pod.Labels[socketquadLabelKey]; !exists {
		klog.V(2).Infof("blindly approving Pod '%s/%s' as it does not have SocketQuadPlugin's label %q", pod.Namespace, pod.Name, socketquadLabelKey)
		return framework.NewStatus(framework.Success, "pod is not tracked by SocketQuadPlugin")
	}

	// For the Node at hand, find all occupant Pods tracked by SocketQuadPlugin.
	...
	// If the Node is full (i.e., 4 applications tracked by SocketQuadPlugin are
	// already scheduled on it), filter it out.
	case 4:
		klog.V(2).Infof("filtering Node %q out because 2 SocketQuadPlugin applications are already scheduled there", nodeName)
		return framework.NewStatus(framework.Unschedulable, fmt.Sprintf("Node '%s' already has 2 SocketQuadPlugin occupants", nodeName))
	...
	case 3:
		...
		if score > sla {
			msg := fmt.Sprintf("filtering Node '%s': new pod '%s/%s' ('%s') incurs huge slowdown on pod '%s/%s' ('%s')",
				nodeName, pod.Namespace, pod.Name, pod.Labels[socketquadLabelKey],)
			klog.V(2).Infof(msg)
	...
	case 1:
		...
		if score > sla {
			msg := fmt.Sprintf("filtering Node '%s': new pod '%s/%s' ('%s') incurs huge slowdown on pod '%s/%s' ('%s')",
				nodeName, pod.Namespace, pod.Name, pod.Labels[socketquadLabelKey], occ.Namespace, occ.Name, occ.Labels[socketquadLabelKey])
			klog.V(2).Infof(msg)
			return framework.NewStatus(framework.Unschedulable, msg)
		}
		fallthrough
...
//
// In the case of SocketQuadPlugin, scoring is reversed; i.e., higher score indicates
// worse scheduling decision.
// This is taken into account and "fixed" later, during the normalization.
func (ap *SocketQuadPlugin) Score(
	ctx context.Context,
	state *framework.CycleState,
	p *corev1.Pod,
	nodeName string,
...
// ScoreExtensions returns the SocketQuadPlugin itself, since it implements the
...
//
// In the case of the SocketQuadPlugin, its "Score" method produces scores of reverse
...
func (_ *SocketQuadPlugin) NormalizeScore(
	ctx context.Context,
	state *framework.CycleState,
	p *corev1.Pod,
	...
	// When no Pod (tracked by SocketQuadPlugin) is scheduled on the Node,
	// maxScore will be 0.
	if maxScore == 0 {
		for i := range scores {
